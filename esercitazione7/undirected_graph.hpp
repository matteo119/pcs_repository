...
public:
	...
	undirected_edge edge_at(const int number) const {
		int i=0;
	...
	int edge_number(const undirected_edge& edge) const {
		int i=0;
		for (const auto& e : edges) {
			if (e==edge) return i;
			i++;
	...
	std::set<int> neighbours(const int node) const {
		if (neighborhood.contains(node)) return neighborhood.at(node);
		return {};
	}
